...
/**
 * An {@link BasicAuthenticationDetailsProvider} implementation that uses
 * workload identity federation
 * to authenticate with Oracle Cloud Infrastructure. This provider exchanges a
 * subject token (@see https://www.rfc-editor.org/rfc/rfc8693.html#section-2.1)
 * for an OCI session token, which is then used to sign API requests.
 *
 * <p>
 * This provider offers two key features for robust authentication in
 * long-running applications:
 * <ol>
 * <li><b>Asynchronous Initialization with {@code buildAsync()}:</b><br>
 * The {@link #builder()} provides a {@code buildAsync()} method that
 * pre-fetches the first
 * authentication token upon initialization. This "fail-fast" approach ensures
 * that authentication issues are discovered at startup rather than during the
 * first
 * API call, and it eliminates the initial authentication delay.</li>
 *
 * <li><b>Automatic Proactive Token Refresh with
 * {@code retryConfiguration()}:</b><br>
 * For applications that make continuous API calls, this provider offers
 * automatic proactive token refresh when retry configuration is provided via
 * {@link WorkloadIdentityFederationAuthenticationDetailProviderBuilder#retryConfiguration(RetryConfiguration)}.
 * When retry configuration is set, the provider uses a background thread to
 * automatically
 * refresh the session token before it expires, preventing the calling thread
 * from being blocked
 * by token refresh operations and ensuring consistent API call performance.
 * When no retry
 * configuration is provided, proactive refresh is disabled to conserve
 * resources.</li>
 * </ol>
 *
 * <p>
 * When proactive refresh is enabled (via retry configuration), it is crucial to
 * call {@link #close()} (or {@link #shutdown()})
 * when the provider is no longer needed to release the background scheduling
 * thread.
 *
...
public class WorkloadIdentityFederationAuthenticationDetailProvider
    ...
    private static final Logger LOG = org.slf4j.LoggerFactory
            .getLogger(WorkloadIdentityFederationAuthenticationDetailProvider.class);
    ...
    public static class WorkloadIdentityFederationAuthenticationDetailProviderBuilder {
         ...
         * <p>
         * When a retry configuration is provided, this builder will also enable
         * proactive background refresh.
         * The retry configuration is passed through to the token exchange
         * {@code ClientCall}.
         * </p>
         *
         ...
         * Builds the {@link WorkloadIdentityFederationAuthenticationDetailProvider}
         * asynchronously.
         * This method initializes the federation client and pre-fetches the first
         * token,
         * then returns a CompletableFuture that completes with the fully initialized
         * provider.
         *
         * This is useful when you want to ensure the provider is ready with a valid
         * token
         * before passing it to a client that needs authentication.
         *
         * @throws IllegalArgumentException if any required field is not set.
         * @return a CompletableFuture containing the initialized authentication
         *         provider
         */
        public CompletableFuture<WorkloadIdentityFederationAuthenticationDetailProvider> buildAsync() {
            ...
            WorkloadIdentityFederationAuthenticationDetailProvider provider = new WorkloadIdentityFederationAuthenticationDetailProvider(
