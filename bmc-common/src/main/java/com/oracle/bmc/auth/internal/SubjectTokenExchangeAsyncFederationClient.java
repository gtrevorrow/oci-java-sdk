...
public class SubjectTokenExchangeAsyncFederationClient extends AbstractAsyncFederationClient {
                    ...
                    .whenComplete(
                            (newAdapter, error) -> {
                                if (error != null) {
                                    ...
                                    future.complete(this.securityTokenAdapter.getSecurityToken());
                                }
                            });
            return future;
        }
    ...
    public CompletableFuture<SecurityTokenAdapter> getSecurityTokenFromServer() {
        ...
        try {
            ...
            String basicAuth = java.util.Base64.getEncoder().encodeToString((clientCredentials).getBytes());
            ...
            requestBodyBuilder
                    ...
                    .append(
                            java.net.URLEncoder.encode(
                                    "urn:ietf:params:oauth:token-type:jwt", "UTF-8"));
            requestBodyBuilder
                    .append("&subject_token=")
                    .append(java.net.URLEncoder.encode(subjectToken, "UTF-8"));
            requestBodyBuilder
                    .append("&requested_token_type=")
                    .append(java.net.URLEncoder.encode("urn:oracle:iam:token-type:upst", "UTF-8"));
            // DER base64 encode the public key
            String publicKeyDerBase64 = java.util.Base64.getEncoder().encodeToString(publicKey.getEncoded());
            requestBodyBuilder
                    .append("&public_key=")
                    .append(java.net.URLEncoder.encode(publicKeyDerBase64, "UTF-8"));
            requestBodyBuilder
                    .append("&scope=")
                    .append(java.net.URLEncoder.encode("urn:oracle:iam::scope:workload", "UTF-8"));
                    ...
                    .thenCompose(
                            response -> {
                                ...
                                return response.textBody()
                                        .thenApply(
                                                body -> {
                                                    Serializer ser = HttpProvider.getDefault()
                                                            .getSerializer();
                                                    try {
                                                        com.fasterxml.jackson.databind.JsonNode rootNode = ser
                                                                .readValue(
                                                                        body,
                                                                        com.fasterxml.jackson.databind.JsonNode.class);
                                                        String accessToken = rootNode.get("access_token")
                                                                .asText();
                                                        LOG.debug(
                                                                "Token exchange response: {}",
                                                                body);
                                                        return new SecurityTokenAdapter(
                                                                accessToken, sessionKeySupplier);
